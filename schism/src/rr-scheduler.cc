...
void RRScheduler::add_sender( SlottedSender const * sender, uint32_t weight )
{
...
SlottedSender * RRScheduler::get_sender_for_slot( uint32_t slot )
{
...
Packet RRScheduler::get_next_packet()
{
	...
	if( rr_index == _num_slots ) {
		...
		return _current_sender->send_packet();
	}
}
